...
**Schema must match the handoff's mock data shapes.** Read `src/design-handoff/` to find the hardcoded data structures the frontend expects. Column names should match mock object property names where possible. If a mock shape requires transformation (denormalized, computed), document it as a database view or in the data hook.
