...
- [ ] Design tool account — per the adapter selected in `artifacts/design-tool.config.json`. Default adapter (`claude-design`) requires [Claude](https://claude.ai) Pro/Max/Team/Enterprise. See the adapter's SKILL.md in `.cursor/skills/design-adapters/` for specific requirements.
...
| 7 | Build Scope + Landing Content | Feature list, design brief copy |
...
2. **Design brief** — structured prompt with brand context, anti-patterns, and per-screen content hierarchy, formatted for whichever AI design tool the project uses

**Gate:** Review both files → approve.

---

## Phase 3 — Design (via configured adapter)

RAD is tool-agnostic at this phase. The adapter configured in `artifacts/design-tool.config.json` owns the human workflow. Read `.cursor/skills/design-adapters/[adapter-name]/SKILL.md` for prompting patterns, export checklist, and failure modes specific to your tool.

**The adapter produces a canonical handoff at `src/design-handoff/` matching `artifacts/docs/handoff-contract.md` v3.** Every adapter produces the same shape — that's what lets the downstream pipeline be tool-agnostic.

**When the adapter completes:**
1. Human exports/produces the handoff per the adapter's SKILL.md
2. Run `/design verify` — canonical checks + adapter-specific checks (gates C2/C3/H1/H2/H3/H4 run when `claude-design-adapter` is active)
3. On pass, proceed to `/phase4`
...
5. Extract entities from the handoff's mock data + northstar
...
- Move the handoff's `components/ui/` → `src/components/ui/` (as-is)
- Catalog components + build missing states (EmptyState, ErrorBanner, SkeletonCard)
- Port the adapter's CSS design tokens into `src/app.css`, self-host fonts
...
- Screens are built incrementally from handoff reference TSX; see `mobile-developer` agent and `/native-init` command
...
**Handoff → React Native:** Hybrid translation is **risk-graded** (queue approved per screen), uses an **import audit** before porting, forbids **handoff mock data** on device after wiring (only **`shared/`** hooks), and defines **escalation** when mechanical mapping fails — see **`.cursor/agents/mobile-developer.md`** (*Handoff → RN*).

The frontend agent is an integrator, not a builder:
- Finds the handoff component for each screen
...
- Keeps everything else the adapter generated — layout, styling, animations
...
| Dimension | Weight |
|---|---|
| Visual fidelity (handoff spec match) | 15% |
...
Product Designer checks every screen against the canonical handoff's original components, slop guard rules, mobile viewport, interaction states, copy quality, and landing page completeness.
...
flowchart TD
    ...
    C["/phase2\nScreen Specs + Design Brief"]
    C2["Configured design tool\n(human-driven design, per adapter)"]
    ...
    C -->|"design via configured tool"| C2
...
| `/phase2` | Product Designer | Screen specs + design brief |
