...
This is the canonical design system doc for [App Name]. Produced by the active design adapter during `/foundation` or `/design`. Frontend agents read this alongside every screen spec when porting. Structure per `artifacts/docs/handoff-contract.md` §Design context document.
